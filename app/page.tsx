...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((error) => {
        console.error("Failed to load Sanity stories:", error);
      });
  }, []);
  ...
  const handleStepInside = (e: React.MouseEvent<HTMLButtonElement>) => {
    // Alt/meta-click opens the top featured story as a portal.
    if (e?.altKey || e?.metaKey) {
      if (featured[0]) router.push(`/read/${encodeURIComponent(featured[0].id)}`);
      return;
    }

    // Default: smoothly scroll to mood categories
  ...
  useEffect(() => {
    const calc = () => {
      ...
      // pull the scene left on small screens to avoid the chair on the right
        ...
        <main className="relative z-10 pt-14 pb-6 px-4 max-w-lg mx-auto">
          <div className="relative z-10 max-w-sm">
            ...
            <div className="mt-4 space-y-3 text-slate-200">
              <p className="font-body text-base italic leading-relaxed drop-shadow-[0_10px_25px_rgba(0,0,0,0.22)]">
                {ambientMessage}
              </p>
              <p className="font-body text-base italic leading-relaxed drop-shadow-[0_10px_25px_rgba(0,0,0,0.22)]">
